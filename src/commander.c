...
// To send the selector command, we first get a,b,x,y button status.
static int command_selector() {
    int return_code = OSCC_ERROR;
    int return_code_p = OSCC_ERROR;
    int return_code_r = OSCC_ERROR;
    int return_code_n = OSCC_ERROR;
    int return_code_d = OSCC_ERROR;

    static char request_position = 0x00;

    if (commander_enabled == COMMANDER_ENABLED && control_enabled == true) {
        unsigned int p_button_current = 0;
        unsigned int r_button_current = 0;
        unsigned int n_button_current = 0;
        unsigned int d_button_current = 0;

        return_code_p = get_button(JOYSTICK_BUTTON_P_SELECTOR_POSITION,
                                   &p_button_current);

        return_code_r = get_button(JOYSTICK_BUTTON_R_SELECTOR_POSITION,
                                   &r_button_current);

        return_code_n = get_button(JOYSTICK_BUTTON_N_SELECTOR_POSITION,
                                   &n_button_current);

        return_code_d = get_button(JOYSTICK_BUTTON_D_SELECTOR_POSITION,
                                   &d_button_current);

        if (return_code_p == OSCC_OK &&
            return_code_r == OSCC_OK &&
            return_code_n == OSCC_OK &&
            ...
            if ((p_button_current + p_button_current + p_button_current + p_button_current) == 1) {
                if (p_button_current) {
                    ...
                    request_position = 'R';
                } else if (n_button_current) {
                    request_position = 'N';
                } else if (d_button_current) {
                    request_position = 'D';
                }
